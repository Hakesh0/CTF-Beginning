...
const GobusterPage = () => {
  ...
  const [command, setCommand] = useState('gobuster dir -u http://target/FUZZ -w wordlist.txt');
  ...
  const handleFuzzing = async () => {
    ...
    try {
      const tool = "gobuster";
      const filePath = path.join(outputPath, `gobuster.txt`);
  ...
  return (
    <div className="flex justify-center items-start h-full pt-8">
      <Card className="w-full max-w-5xl">
        <CardHeader>
          <CardTitle>Gobuster</CardTitle>
          <CardDescription>Perform fuzzing using gobuster.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4">
          <div className="grid gap-2">
            <Label htmlFor={`target-gobuster`}>Target</Label>
            <Input
              id={`target-gobuster`}
          ...
          <div className="grid gap-2">
            <Label htmlFor={`command-gobuster`}>Command</Label>
            <Input
              id={`command-gobuster`}
          ...
          <div className="grid gap-2">
            <Label htmlFor={`outputPath-gobuster`}>Output File Path</Label>
            <Input
              id={`outputPath-gobuster`}
